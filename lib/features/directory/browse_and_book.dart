...
class _BrowseAndBookPageState extends State<BrowseAndBookPage> {
  ...
  void _openBookForm(DoctorProfile d, DutySchedule? schedule) {
    final center = _center;
    showDialog<void>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Contact ${d.fullName}'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            if (center != null) ...[
              const SizedBox(height: 8),
              Text('Medical Center: ${center.name}'),
              if (center.phone?.isNotEmpty ?? false)
                Text('Phone: ${center.phone}'),
              if (center.email?.isNotEmpty ?? false)
                Text('Email: ${center.email}'),
            ],
            const SizedBox(height: 12),
            const Text(
              'Please contact the medical center directly to schedule an appointment.',
...
class _BookDialogState extends State<_BookDialog> {
  ...
  void _submit() {
    // Booking functionality removed — appointment feature has been disabled.
